...
function AddContact ( { setShowAddContact } )
{
    ...
    return (
        <div className=" h-full bg-black flex flex-col justify-start items-center gap-10">
            ...
            <div className="w-full bg-[#2c2c2e] py-1 px-2 h-fit flex flex-col justify-start items-center gap-2">
                {addPhone.map( ( item ) => (
                    <div key={item.id} className="w-full flex justify-start items-center gap-1 h-fit">
                        <button onClick={() => handleMinusPhone( item.id )}><img src={minus} alt="minus" className="w-[40px]" /></button>
                        <h3 className="text-[#007aff] text-sm">mobile</h3>
                        <i className="fa-solid fa-chevron-right ml-0.5 text-[10px] text-[#a7a7a7]"></i>
                        <input type="number" className="bg-transparent w-full h-[40px] ml-2 border-l border-l-[#1bc47d] outline-none px-3 text-white" placeholder="Phone" />
                    </div>
                ) )}
                <div className="w-full h-fit flex justify-start items-center gap-1" onClick={handleAddPhone}>
